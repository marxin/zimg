	...
	static size_t calculate_tmp_size(const SimulationState &sim, const std::vector<std::unique_ptr<GraphNode>> &nodes, const GraphNode *sink)
	{
		...
		for (const auto &node : nodes) {
			...
			for (int p = 0; p < PLANE_NUM; ++p) {
				...
				unsigned shift_w = p == PLANE_U || p == PLANE_V ? node->get_subsample_w() : 0;
				unsigned shift_h = p == PLANE_U || p == PLANE_V ? node->get_subsample_h() : 0;

				checked_size_t rowsize = checked_size_t{ attr.width >> shift_w } * pixel_size(attr.type);
				checked_size_t stride = ceil_n(rowsize, ALIGNMENT);
		...
		m_tmp{}
	{
		...
		for (const auto &node : nodes) {
			...
			for (int p = 0; p < PLANE_NUM; ++p) {
				...
				size_t rowsize = static_cast<size_t>(attr.width >> shift_w) * pixel_size(attr.type);
				size_t stride = ceil_n(rowsize, ALIGNMENT);
				...
				buffer[p] = { alloc.allocate(stride * plane_lines), static_cast<ptrdiff_t>(stride), suggestion.second };
			}
		}
...
public:
	...
	{
		return (1U << m_source->get_subsample_h()) - 1;
	}

	unsigned get_output_buffering() const
	{
		return (1U << m_sink->get_subsample_h()) - 1;
	}
